...
int16_t shoot_can_set_current_17mm = 0;
...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        shoot_can_set_current = shoot_control_loop();        //射击任务控制循环
				shoot_can_set_current_17mm = shoot_control.given_current_17mm;
#if YAW_TURN
...
//--------------------------------------------------
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            {
                //CAN_cmd_gimbal(0, 0);
							  //CAN_cmd_gimbal2(0,0,0);
							  CAN_cmd_gimbal_new_Hero_gimbal(0,0,0,0);
            }
            else
            {
                //CAN_cmd_gimbal(pitch_can_set_current, 0);
							//CAN_cmd_gimbal2(shoot_can_set_current,yaw_can_set_current, shoot_can_set_current_17mm);
								CAN_cmd_gimbal_new_Hero_gimbal(pitch_can_set_current, yaw_can_set_current, shoot_can_set_current, shoot_can_set_current_17mm);
							//CHANGE ABOVE FUNCTION BACK
						}
        }
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add, uint8_t bypass)
{
    ...
    //当前控制误差角度
		...
		if(bypass == 0)
		{
			...
			/*5 度 => 5 * PI /180  = 0.08726f*/
			// /*
			if (gimbal_motor->relative_angle + bias_angle + add > (gimbal_motor->max_relative_angle+0.0870f))//gimbal_motor->max_relative_angle)
			{
					//如果是往最大机械角度控制方向
					if (add > 0.0f)
					{
							...
							//计算出一个最大的添加角度，
							add = (gimbal_motor->max_relative_angle+0.0870f) - gimbal_motor->relative_angle - bias_angle;
					}
			}
			else if (gimbal_motor->relative_angle + bias_angle + add < (gimbal_motor->min_relative_angle-0.88f))//gimbal_motor->min_relative_angle)
			{
					if (add < 0.0f)
					{
							add = (gimbal_motor->min_relative_angle-0.88f) - gimbal_motor->relative_angle - bias_angle;
					}
			}
			// */
		}
